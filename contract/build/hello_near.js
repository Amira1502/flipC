...
let coinFlip = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view({}), _dec(_class = (_class2 = class coinFlip {
  ...
  flipCoin({
    ...
    log(`${player} chose ${guess_coin}`);
    // Simulate coin flip
    const outcome = generateRandomeNb();

    // Get the current player points
    let player_points = this.points.get(player) || 0;
...
}, (_applyDecoratedDescriptor(_class2.prototype, "flipCoin", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "flipCoin"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "viewPoints", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "viewPoints"), _class2.prototype)), _class2)) || _class);
function viewPoints() {
  let _state = coinFlip._getState();
  if (!_state && coinFlip._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  let _contract = coinFlip._create();
  if (_state) {
    coinFlip._reconstruct(_contract, _state);
  }
  let _args = coinFlip._getArgs();
  let _result = _contract.viewPoints(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(coinFlip._serialize(_result));
}
function flipCoin() {
  let _state = coinFlip._getState();
  if (!_state && coinFlip._requireInit()) {
  ...
  let _args = coinFlip._getArgs();
  let _result = _contract.flipCoin(_args);
  coinFlip._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(coinFlip._serialize(_result));
}

export { flipCoin, viewPoints };
